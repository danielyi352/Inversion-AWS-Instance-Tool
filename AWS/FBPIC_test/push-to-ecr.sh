#!/usr/bin/env bash
#
# Push GPU Docker Image to Amazon ECR
# This script builds and pushes the GPU Docker image to Amazon ECR.
...
readonly ECR_REPOSITORY="gpu-fbpic"
...
build_and_push_image() {
  ...
  docker build --platform linux/amd64 -f FBPIC.dockerfile -t "${FULL_IMAGE_NAME}" .
...
main() {
  log "Starting GPU-fbpic Docker Image Push to ECR..."
  check_aws_cli
  check_docker
  create_ecr_repository
  authenticate_ecr
  build_and_push_image
  log "GPU-fbpic Docker Image Push to ECR completed successfully."
